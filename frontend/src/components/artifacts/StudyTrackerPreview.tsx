...
export default function StudyTrackerPreview({ fixture }: { fixture: unknown }) {
  ...
  const [client] = useState(() => {
    ...
    return new ArtifactClient(async (path, body) => {
      if (body && path.startsWith("state?")) {
        ...
        const enabled = (body as { edit: { enabled: boolean } }).edit.enabled;
  ...
  return <main style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#04080f", color: "#e2e8f0" }}>
    ...
    <div style={{ flex: 1, minHeight: 0, minWidth: 0 }}>
      {owner ? <ArtifactCanvas content={view.artifact ? { kind: "generated_ui", artifact: view.artifact } : null} onArtifactEdit={client.edit} artifactBusy={view.busy} onDismiss={client.dismiss} /> : <p role="status" style={{ padding: 24 }}>Private tracker removed. Owner return requires a fresh load.</p>}
    </div>
  </main>;
}
